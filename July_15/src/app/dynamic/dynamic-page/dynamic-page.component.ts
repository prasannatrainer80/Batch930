...
import { APICallService, CommonService, MasterBase, MasterService } from '@core';
...
export class DynamicPageComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  ...
  constructor(
    ...
    this.activatedRoute.paramMap.subscribe((params: any) => {
      ...
      this.actionType = common.isEmpty(this.itemCode) ? 'insert' : 'update';

      let rouePath = this.activatedRoute?.snapshot?.routeConfig?.path || '';
      if (rouePath.toLowerCase().indexOf('view/') > -1) {
  ...
  init() {
    this.loading = true;
    this.apiCall.post(`/dataapi/config/get`, { module: this.moduleName, action: this.actionType }).subscribe((pagedData: any) => {

      if (!this.common.isEmptyObj(pagedData?.breadcrumb)) {
        this.breadscrums = pagedData?.breadcrumb;
        this.pageTitle = this.breadscrums[0]?.active;
        this.cancelURL = this.breadscrums[0]?.items[this.breadscrums[0]?.items?.length - 1]?.url;
      }
  ...
  scrollToElement(index: number): void {
    ...
    element.scrollIntoView({ behavior: 'smooth', duration: 2000, easing: 'linear', block: 'center' });
  }
  setCardHeight() {
  ...
  async submitForm() {
    ...
    payLoad = { ...payLoad, ...{ module: this.moduleName, action: this.actionType } };
